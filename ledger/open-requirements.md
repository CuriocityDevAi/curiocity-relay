...
- [ ] K1: /api/gate를 auto-merge.yml에 연결(기계 빨강이면 도장 불가 · K2 gate-contract 기준)
...
- [ ] K2: 테스트 체계 v1 원칙 문서(수용 기준 코드 전 · 층 배치 · 환경 동일성 · 게이트 · 이력 · 소유) + Kyu 5항(요구 번호로 테스트 찾기 · 요구 변경 시 같은 PR에서 테스트 수정 · 싼 테스트 전량/비싼 테스트 선택+머지 전 전량 · 사람 판정 최소 = 백엔드/문서만이면 0건 자동 배포 · 배포 기준 4조건)
